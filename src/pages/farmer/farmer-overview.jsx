...
const FarmerDashboard = () => {
  ...
  // Orders from state.order (same as My Orders page)
  const { orders = [], isLoading: isLoadingOrders } = useSelector((s) => s.order || {})
  // Recommendations from state.recommendation (your slice)
  const {
    myRecommendations = [],
    isLoading: isLoadingRecs,
    error: recError,
  } = useSelector((s) => s.recommendation || {})
  ...
  // Same badge logic as your FarmerRecommendations page
  ...
  // Pending recommendations: in_review + pending_review
  const pendingRecommendationsCount = useMemo(() => {
    return (myRecommendations || []).filter(r =>
      ['in_review', 'pending_review'].includes((r.status || '').toLowerCase())
    ).length
  }, [myRecommendations])
  ...
  // Monthly recs (last 6 months) using timestamp (fallback created_at/updated_at)
  const recommendationMonthlyData = useMemo(() => {
    const months = lastNMonths(6)
    const countsByYM = (myRecommendations || []).reduce((acc, r) => {
      const when = r.timestamp || r.created_at || r.updated_at
      const key = toYM(when)
      if (!key) return acc
      acc[key] = (acc[key] || 0) + 1
      return acc
  ...
  const dailyActivityData = useMemo(() => {
    ...
    const recsByDay = (myRecommendations || []).reduce((acc, r) => {
      const k = toDateKey(r.timestamp || r.created_at || r.updated_at)
      if (!k) return acc
      acc[k] = (acc[k] || 0) + 1
      return acc
    }, {})
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="divide-y divide-gray-100">
                {isLoadingRecs ? (
                  ...
                  (myRecommendations || []).slice(0, 3).map((rec, idx) => {
                    const status = (rec.status || '').toLowerCase()
                    const crop = rec.crop_predicted || rec.ai_outputs?.crop_predicted || 'Unknown'
                    const stamp = rec.timestamp || rec.created_at || rec.updated_at
                    return (
                      <div key={rec.id || idx} className="p-6 hover:bg-gray-50 transition-colors">
                        <div className="flex justify-between items-start mb-3">
                          <div className="flex-1">
                            ...
                            <div className="flex flex-wrap gap-3 mt-2 text-sm text-gray-600">
                              <span className="flex items-center gap-1"><FiCalendar /> {formatDate(stamp)}</span>
                              ...
                              <span className={`px-2 py-1 rounded-full border text-xs ${getRecStatusBadge(rec.status)}`}>
                                {(rec.status || 'pending_review').replace("_", " ")}
                              </span>
                            </div>
                          </div>
                          <div className="ml-4">
                        ...
                        {['translated','returned'].includes(status) && rec.translated_summary && (
