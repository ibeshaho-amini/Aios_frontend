...
import { updateOrderStatus } from "../../Redux/order/orderSlice" // NEW
...
export default function SupplierOrdersPage() {
  ...
  const [actingId, setActingId] = useState(null) // `${orderID}:${status}` while updating
  ...
  const supplierOrders = useMemo(() => {
    ...
    return (orders || []).filter((o) => Number(o?.supplier) === supplierUserId)
  }, [orders, supplierUserId])
  ...
  const filteredOrders = useMemo(() => {
    ...
    return supplierOrders.filter((o) => {
      ...
      if (!text) return true
      const buyer = (o?.user_username || "").toLowerCase()
      const oid = String(o?.orderID || "").toLowerCase()
      const st = (o?.status || "").toLowerCase()
      return buyer.includes(text) || oid.includes(text) || st.includes(text)
    })
  }, [supplierOrders, statusFilter, q])
  ...
  const doUpdate = async (orderID, nextStatus) => {
    setActingId(`${orderID}:${nextStatus}`)
    const res = await dispatch(updateOrderStatus({ orderID, status: nextStatus }))
    setActingId(null)
    // If modal is open for this order, refresh its data from the list
    if (viewOrder?.orderID === orderID && !res.error) {
    ...
    return (
      <div className="flex items-center justify-end gap-2">
        {s === "pending" && (
          <>
            <button
              onClick={() => doUpdate(o.orderID, "approved")}
              className="px-3 py-1.5 bg-green-50 text-green-700 hover:bg-green-100 rounded-lg text-sm"
              disabled={actingId === `${o.orderID}:approved` || isLoading}
            >
              {actingId === `${o.orderID}:approved` ? "Approving…" : "Approve"}
            </button>
            <button
              onClick={() => doUpdate(o.orderID, "rejected")}
              className="px-3 py-1.5 bg-red-50 text-red-700 hover:bg-red-100 rounded-lg text-sm"
              disabled={actingId === `${o.orderID}:rejected` || isLoading}
            >
              {actingId === `${o.orderID}:rejected` ? "Rejecting…" : "Reject"}
            </button>
          </>
        )}
        {s === "approved" && (
          <button
            onClick={() => doUpdate(o.orderID, "completed")}
            className="px-3 py-1.5 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 rounded-lg text-sm"
            disabled={actingId === `${o.orderID}:completed` || isLoading}
          >
            {actingId === `${o.orderID}:completed` ? "Completing…" : "Mark as Completed"}
          </button>
        )}
        <button
          onClick={() => openModal(o)}
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white shadow rounded-2xl p-5 mb-6 border border-white/50">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-gray-800">Supplier Orders</h1>
              <p className="text-sm text-gray-600">View and manage orders placed to you</p>
            </div>
            <button
            ...
            <table className="min-w-full divide-y divide-gray-100">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="text-left text-xs font-semibold text-gray-600 uppercase px-4 py-3">Buyer</th>
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                {isLoading && filteredOrders.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="text-center px-4 py-10 text-blue-600">
                      Loading orders…
                    </td>
                  </tr>
                    ...
                    return (
                      <React.Fragment key={o.orderID}>
                        <tr className="hover:bg-gray-50/60">
                          ...
                          <td className="px-4 py-3 text-gray-800">@{o.user_username || o.user}</td>
                          <td className="px-4 py-3 text-gray-700">{fmtDateTime(o.orderDate)}</td>
                          <td className="px-4 py-3">
                            <span className={badgeByStatus(o.status)}>{o.status}</span>
                          </td>
...
function OrderDetailsModal({ order, onClose, doUpdate, actingId, isLoading }) {
          ...
          <div className="p-4 border-b flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">
                Buyer @{order.user_username || order.user} • {fmtDateTime(order.orderDate)}
              </p>
            </div>
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="flex items-center justify-end gap-2 pt-2">
              {s === "pending" && (
                <>
                  <button
                    onClick={() => doUpdate(order.orderID, "approved")}
                    className="px-3 py-1.5 bg-green-50 text-green-700 hover:bg-green-100 rounded-lg text-sm"
                    disabled={actingId === `${order.orderID}:approved` || isLoading}
                  >
                    {actingId === `${order.orderID}:approved` ? "Approving…" : "Approve"}
                  </button>
                  <button
                    onClick={() => doUpdate(order.orderID, "rejected")}
                    className="px-3 py-1.5 bg-red-50 text-red-700 hover:bg-red-100 rounded-lg text-sm"
                    disabled={actingId === `${order.orderID}:rejected` || isLoading}
                  >
                    {actingId === `${order.orderID}:rejected` ? "Rejecting…" : "Reject"}
                  </button>
                </>
              )}
              {s === "approved" && (
                <button
                  onClick={() => doUpdate(order.orderID, "completed")}
                  className="px-3 py-1.5 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 rounded-lg text-sm"
                  disabled={actingId === `${order.orderID}:completed` || isLoading}
                >
                  {actingId === `${order.orderID}:completed` ? "Completing…" : "Mark as Completed"}
                </button>
              )}
              <button onClick={onClose} className="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 rounded-lg text-sm">
                Close
              </button>
            </div>

            <div className="text-sm text-gray-500">
              Supplier (you): @{order.supplier_username || order.supplier}
            </div>
          </div>
        </div>
      </div>
